...
```{r}
#subarea_categories <- unique(iowa_roads$CARS.SubArea)
...
category_freq <- iowa_roads %>%
  count(CARS.SubArea)

# Determine the number of categories per section
categories_per_section <- ceiling(nrow(category_freq) / 11)

# Assign a section number to each category based on frequency rank
category_freq <- category_freq %>%
  mutate(section = rep(1:11, each = categories_per_section, length.out = n()))

# Merge section information back to the original dataframe
iowa_roads <- iowa_roads %>%
  left_join(category_freq, by = "CARS.SubArea")

# Plot using facet_wrap with the 'section' variable
